...
CATEGORIAS_BEBIDAS_REPORTE = {'Bebidas', 'Desayuno'}

BEBIDAS_CATALOGO = {
    'gaseosa': {'nombre': 'Gaseosa', 'precio': 3500.0},
    'agua': {'nombre': 'Agua', 'precio': 3000.0},
    'saborizada': {'nombre': 'Saborizada', 'precio': 3300.0},
    'ficha_de_cafe': {'nombre': 'Ficha de Cafe', 'precio': 2000.0},
    'cerveza': {'nombre': 'Cerveza', 'precio': 5500.0},
    'nampe_malbec': {'nombre': 'Nampe Malbec', 'precio': 6500.0},
    'chacabuco_chenin': {'nombre': 'Chacabuco Chenin', 'precio': 8000.0},
    'chacabuco_malbec': {'nombre': 'Chacabuco Malbec', 'precio': 7800.0},
    'los_haroldos_reserva_malbec': {'nombre': 'Los Haroldos Reserva Malbec', 'precio': 14800.0},
    'los_haroldos_reserva_chardonnay': {'nombre': 'Los Haroldos Reserva Chardonnay', 'precio': 14800.0},
    'los_haroldos_estate_malbec': {'nombre': 'Los Haroldos Estate Malbec', 'precio': 11000.0},
    'los_haroldos_estate_chardonnay': {'nombre': 'Los Haroldos Estate Chardonnay', 'precio': 11000.0},
}
...
def _normalizar_categoria_reporte(categoria):
    """Normaliza categorías para mantener compatibilidad con reportes existentes."""
    return 'Bebidas' if categoria in CATEGORIAS_BEBIDAS_REPORTE else categoria


def _obtener_categoria_mostrar(categoria, item=''):
    """Determina la etiqueta visible de la categoría para el usuario."""
    item_limpio = '' if pd.isna(item) else str(item).strip()
    if categoria == 'Desayuno':
        return 'Desayuno'
    if categoria == 'Bebidas' and not item_limpio:
        return 'Desayuno'
    return categoria


def _obtener_categoria_css(categoria, item=''):
    """Retorna la clase CSS asociada a la categoría mostrada."""
    return _obtener_categoria_mostrar(categoria, item).strip().lower()
...
def _leer_archivo_consumos(archivo_consumos):
    ...
    columnas_ordenadas = CONSUMOS_COLUMNS + [columna for columna in df.columns if columna not in CONSUMOS_COLUMNS]
...
def obtener_total_consumos(num_habitacion, archivo_consumos='data/consumos_diarios.csv'):
    ...
    categorias = consumos['categoria'].astype(str)
    totales = {
        'Bebidas': consumos[categorias.isin(CATEGORIAS_BEBIDAS_REPORTE)]['monto'].sum(),
        'Estadía': consumos[categorias == 'Estadía']['monto'].sum(),
        'Map': consumos[categorias == 'Map']['monto'].sum(),
    }
...
def obtener_consumos_por_pasajero(num_habitacion, archivo_consumos='data/consumos_diarios.csv'):
    ...
    for _, row in consumos.iterrows():
        ...
        categoria_mostrar = _obtener_categoria_mostrar(row['categoria'], row.get('item', ''))
        categoria_css = _obtener_categoria_css(row['categoria'], row.get('item', ''))
        ...
        consumos_por_pasajero[pasajero]['consumos'].append({
            ...
            'categoria_mostrar': categoria_mostrar,
        ...
        categoria = _normalizar_categoria_reporte(row['categoria'])
...
def obtener_total_consumos_pasajero(num_habitacion, nombre_pasajero, archivo_consumos='data/consumos_diarios.csv'):
    ...
    categorias = consumos_pasajero['categoria'].astype(str)
    totales = {
        'Bebidas': consumos_pasajero[categorias.isin(CATEGORIAS_BEBIDAS_REPORTE)]['monto'].sum(),
        'Estadía': consumos_pasajero[categorias == 'Estadía']['monto'].sum(),
        'Map': consumos_pasajero[categorias == 'Map']['monto'].sum(),
    }
    totales['total'] = sum(totales.values())
...
def agregar_consumo_bebida_rapida(num_habitacion, producto_codigo, cantidad, pasajero, archivo_consumos='data/consumos_diarios.csv'):
    ...
    if not producto:
        return False, 'Producto de bebida no valido'

    if producto['precio'] is None:
        return False, f"El precio de {producto['nombre']} todavia no esta configurado"
    ...
    if not consumos.empty:
        for idx, row in consumos.iterrows():
            ...
            lista_consumos.append({
                ...
                'pasajero': row['pasajero'],
                'categoria': row['categoria'],
                'categoria_mostrar': _obtener_categoria_mostrar(row['categoria'], item),
                'categoria_css': _obtener_categoria_css(row['categoria'], item),
                'item': item,
                'cantidad': cantidad,
